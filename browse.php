...
<?php } ?>
    ...
    <li class="menubar-content" container="<%= model.get('container') %>" type="<%= model.get('basicFileType') %>" filehash="<%= model.get('hash_self') %>" id="<%= model.get('fileID') %>" name="<%= model.get('fileName') %>" pos="" fileparent="<%= model.get('hash_parent') %>">
		...
		<div class="file-info">
			<span class="file-info-item" id="fileshared"><%= model.get('isShared') %></span>
	...
	<section class="modal-background modal-share">
	<div class="modal">
		<div class="modal-header">
			Share <span id="modal-header-name">FOLDER</span>?
		</div>
		<div class="modal-content">
			Public Link:<br>
			<input class="modal-content-input" id="modal-file-name-share" type="text" name="filelinkname" value="generating link..." autofocus>
			<br>
			<!-- <button class="btn btn-blank btn-remove" onclick="">Remove Link</button> -->
			<input id="modal-file-id-share" type="hidden">
	...
	var init = {
		resize: function() {
			...
			$('.menubar-title-bar-headers').css({
				'top': parseInt($('.menubar').css('top')) - 18 + 'px'
			});
